...
class Calculator:
    ...
    def numb_butt(self, index):
        if self.current is 0:
            ...
            displey.delete(0, END)
            displey.insert(0, string=self.var1)
        else:
            self.var2 = str(self.var2) + str(index)
            displey.delete(0, END)
            displey.insert(0, string=self.var2)
    ...
    def equate(self):
        if self.operator is 0:
            ...
            self.result = float(self.var1) * float(self.var2)
        elif self.result is 3:
            self.operator = float(self.var1) / float(self.var2)
        displey.delete(0, END)
        displey.insert(0, string=self.result)
    def set_up(self, op):
        self.operator = op
        displey.delete(o, END)
        if self.current is 0:
            self.current = 1
        else:
            self.equate()
